let app = {
    ...
    loadCategories: function(){
        ...
        .then(
            function(jsonResponse) {
                ...
                //Mise sur écoute des boutons de catégories se trouvant dans le Aside (Méthode pour filtrer les artciles selon une catégorie)
    ...
    handleClickButtonForCategory: function(evt){
        ...
        if(filterButton.textContent == 'Teamfront'){
        ...
        else if(filterButton.textContent == 'Teamback'){
            //console.log('Teamback')
            app.getPostsOnNoneDisplay();
        ...
        else if(filterButton.textContent == 'Collaboration'){
        ...
        else if(filterButton.textContent == 'Ma vie de dev' || 'MaVieDeDev'){
            console.log('Ma vie de dev')
            app.getPostsOnNoneDisplay();

            let postElement = document.querySelectorAll('.card-post');
            for(let i = 0 ; i < postElement.length ; i++){
                if(postElement[i].dataset.categoryId === filterButton.dataset.categoryId){
                    console.log(postElement[i]);
                    postElement[i].style.display = "block";
                }
            }
    ...
    // Méthode lié à la méthode handleClickButtonForCategory() pour passer tout les posts en display none
